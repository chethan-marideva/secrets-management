...
- `Authentication:ApiKey` - API key required for requests (sent as `X-Api-Key`). Prefer environment variables or a secure configuration provider. In development, a random key is generated and logged if omitted.
...
The endpoint returns the secret value in plain text and requires an API key. Use it only in trusted environments; keep it behind TLS, disable caching, restrict network access, or consider exposing short-lived tokens instead of raw secrets.
